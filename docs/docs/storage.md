...
- **[Storage: Write](steps/storage-write.md)** — insert a new item.
- **[Storage: Read](steps/storage-read.md)** — look up items, with an optional single-field filter.
- **[Storage: Update](steps/storage-update.md)** — merge new values into an existing item, by id.
- **[Storage: Delete](steps/storage-delete.md)** — permanently remove an item, by id.
...
The **[Storage Changed](triggers/storage-changed.md)** trigger starts a workflow whenever an item in a chosen Storage is created, updated, or deleted.
