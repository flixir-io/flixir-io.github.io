...
Inserts a new item into a [Storage](../storage.md). The values you provide are validated against that Storage's schema before the item is created.
...
!!! note
    Required fields must be present in `values`, or the step fails validation. Field values are expression-aware, so you can populate them from trigger or step data — e.g. `trigger.payload.title`. See the [Expressions guide](../expressions.md).
